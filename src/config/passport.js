...
const jwtVerify = async (payload, done) => {
  try{
   ...
   //checks if logining user is in database or not from payload.sub used while genrating token during login
   const user= await User.findById(payload.sub);
   if (!user){
    return done(null,false);
   }
   done(null,user) //this user is used later in auth.js authentication file
  } 
  catch(err){
    done(err,false)
  }
